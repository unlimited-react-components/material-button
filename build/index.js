...
var kernel = require('@unlimited-react-components/kernel');
var dynamicss = require('@dynamicss/dynamicss');
...
function createRipple(event, variant, color) {
    ...
    if (variant !== "contained") {
        circle.style.backgroundColor = kernel.hexColorToRGB(kernel.asureColor(kernel.colourNameToHex(color)), 0.4);
    }
    else {
        circle.style.backgroundColor = kernel.hexColorToRGB("#ffffff", 0.4);
    }
...
var MaterialButton = function (props) {
    var _a = kernel.mergeProps(props, MaterialButtonDefaultProps), children = _a.children, onClick = _a.onClick, disabled = _a.disabled, style = _a.style, href = _a.href, variant = _a.variant, color = _a.color, textColor = _a.textColor, className = _a.className, text = _a.text;
    //states
    var _b = react.exports.useState(""), idStyles = _b[0], setIdStyles = _b[1];
    var _c = react.exports.useState(false), styleInjected = _c[0], setStyleInjected = _c[1];
    var _d = react.exports.useState(""), classNameCreated = _d[0], setClassNameCreated = _d[1];
    //effects
    /**
     ...
     * @param text
     */
    var handleInserStyle = function (variant, disabled, color, textColor, class_name, text) {
        ...
        if (!disabled) {
            ...
            switch (variant) {
                case "contained":
                    sheetRules[0].rules = {
                        color: kernel.asureColor(kernel.colourNameToHex(textColor)),
                        backgroundColor: kernel.asureColor(kernel.colourNameToHex(color)),
                    };
                    sheetRules[1].rules = {
                        ":hover": {
                            backgroundColor: kernel.darkerColor(kernel.hexColorToRGB(kernel.asureColor(kernel.colourNameToHex(color)), 1)),
                        },
                    };
                    break;
                case "outlined":
                    sheetRules[0].rules = {
                        border: "1px solid " + kernel.hexColorToRGB(kernel.asureColor(kernel.colourNameToHex(color)), 0.5),
                        color: kernel.asureColor(kernel.colourNameToHex(color)),
                        backgroundColor: "transparent",
                    };
                    sheetRules[1].rules = {
                        ":hover": {
                            border: "1px solid " + kernel.hexColorToRGB(kernel.asureColor(kernel.colourNameToHex(color)), 1),
                            backgroundColor: kernel.hexColorToRGB(kernel.asureColor(kernel.colourNameToHex(color)), 0.085),
                        },
                    };
                    break;
                case "text":
        ...
        var id = dynamicss.DynamiCSS.insertStyleSheet(styleSheet);
        setIdStyles(id);
        if (id !== "") {
            setStyleInjected(true);
        }
    };
    var removeStyle = function (idStyles) {
        if (styleInjected) {
            dynamicss.DynamiCSS.removeStyleSheet(idStyles);
    ...
    //references
    var btn_ref = react.exports.useRef(null);
    var span_ref = react.exports.useRef(null);
    /**
     *
     * @param e event
     */
